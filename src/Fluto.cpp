...
MENU_SCREEN(mainScreen, mainItems,
            ...
            ITEM_COMMAND("Imperial march", play_imperial_march),
            ITEM_COMMAND("Au clair de la lune", play_au_clair_de_la_lune),
            ITEM_COMMAND("Calibrate", calibrate));
...
void play_loose()
{
  start_song();
  play_note(DO5, 600, false);
  play_note(SI4, 600, false);
  play_note(LA4D, 600, false);
  for (uint8_t i = 0; i < 8; i++)
  {
    play_note(LA4, 50, false);
    play_note(250, 50, false);
  }
  play_note(LA4, 100, false);
  stop_song();
}
